...
// RayBalls3.glsl - Enhanced with dynamic camera tracking
...
//
// Enhanced for DISCO project with video morphing and dynamic camera tracking
...
const float exposure = 0.005;  // restored original low baseline  0.002
const float gamma = 2.4;   // 2.0 - 2.6
const float intensity = 100.0;  // optional: you can leave at 100 if you like
//const vec3 ambient = vec3(0.6, 0.8, 1.0) * intensity / gamma;
const vec3 ambient = vec3(1.05, 1.0, 0.95) * (intensity / gamma) * 0.3;
...
CameraAdjustment getDynamicCameraAdjustment(float time) {
    ...
    // Normalize angle to 0-2p range for consistent calculations
    float normalizedAngle = mod(angle, 6.28318530718); // 2p
    
    // Calculate distance factor: 0.0 at closest (angle=0), 1.0 at farthest (angle=p)
    ...
    // Farthest point: angle ˜ p
    float farthestFactor = 0.0;
    float distToFarthest = abs(normalizedAngle - 3.14159265359); // p
...
void main() {
    ...
    if (isVideoMorphPhase(iTime)) {
        ...
        if (hit.material.diffuse > 0.0 || hit.material.specular > 0.0) {
            ...
            // No hit - black background during morph
            fragColor = vec4(0.0, 0.0, 0.0, 1.0);
            return;
        }
    }

    // Phase 3 & 4: 3D scene with enhanced dynamic camera system
    vec2 uv = (v_text * iResolution.xy) / iResolution.xy - vec2(0.5);
    uv.x *= iResolution.x / iResolution.y;
