...
// RayBalls2.glsl - Original RayBalls with simple fullscreen video intro
// Based on original RayBalls.glsl
// 
// Attribution:
...
//
// Enhanced for DISCO project with simple fullscreen video intro
...
// NEW: Calculate camera position with smooth transition from close-up to normal
vec3 getCameraPosition(float time) {
    ...
    if (isIntroPhase(time) || isVideoMorphPhase(time)) {
        ...
        // After transition: normal camera behavior (continues the same angle)
        return vec3(
            originalRadius * sin(camAngle),
            2.5,
            originalRadius * cos(camAngle)
...
void main() {
    ...
    // Phase 3 & 4: 3D scene with smooth camera transition
    vec2 uv = (v_text * iResolution.xy) / iResolution.xy - vec2(0.5);
    uv.x *= iResolution.x / iResolution.y;

    // Use smooth camera transition system
    vec3 camPos = getCameraPosition(iTime);

    // Camera target with smooth transition
    vec3 target;
    if (isCameraTransitionPhase(iTime)) {
        ...
        // Normal phase: look at scene center
