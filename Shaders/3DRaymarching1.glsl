...
void main()
{
    ...
    // NEW SMOOTH TRANSITION SYSTEM
    // Creates fluid transitions instead of abrupt jumps
    // Total cycle: 6 seconds (1.5s grow + 4.5s hold)
    // - 0.0-1.5s: Smooth growth to full screen using easeInOutCubic
    // - 1.5-6.0s: Hold full screen state for video viewing
    // - Repeat: Continuous loop with smooth transitions
    float totalCycle = 6.0;
    float cycle = mod(animTime, totalCycle);

    // Phase 1: Growth phase (0.0 to 1.5 seconds)
    float growthDuration = 1.5;
    float holdDuration = 4.5;

    // Calculate smooth transition factor
    float transitionFactor = 0.0;

    if (cycle <= growthDuration) {
        // Growing to full screen (0.0 to 1.0 over 1.5 seconds)
        float growthProgress = cycle / growthDuration;
        // Use smooth easing for natural, fluid growth
        transitionFactor = easeInOutCubic(growthProgress);
    } else if (cycle <= (growthDuration + holdDuration)) {
        // Holding at full screen (1.0 for 4.5 seconds)
        transitionFactor = 1.0;
    } else {
        // This shouldn't happen with our 6-second cycle, but safety fallback
        transitionFactor = 0.0;
    }

    // Main camera animation with user controls (normal 3D movement)
